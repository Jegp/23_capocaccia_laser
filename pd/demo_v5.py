...
with USBInput(dvsres, device="cuda") as stream:
    with laser.Laser() as l:
        ...
        gautensor = torch.zeros(avgresolution, device="cuda")
        ...
        kernel_size = 10
        gaukernel = torch.Tensor(gkern(kernlen=kernel_size, std=6)).to("cuda")
        xlimit = avgresolution[0]-1-kernel_size//2
        ylimit = avgresolution[1]-1-kernel_size//2
