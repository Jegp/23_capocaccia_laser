...
memory = ReplayMemory(REPLAY_SIZE) #10000
...
def plot_durations(show_result=False):
    ...
    #         display.display(plt.gcf())
def optimize_model():
    if len(memory) < LENGTH_MIN2:
        return

    # if(not once):
    #     print("YO!")
    #     once = True
    transitions = memory.sample(BATCH_SIZE) #store transition in memory
    batch=Transition(*zip(*transitions)) #store transition in batch
    non_final_mask = torch.tensor(tuple(map(lambda s: s is not None, batch.next_state)), device=device, dtype=torch.bool) #will basically create a tensor with all the index of non none values of batch.next_state (lambda s = a function s which takes batch.next_state as value if s is not none).
    #map takes the index of batch.next_state for non-none states. tuple transform the iterable to a tuple in order to keep the index. torch.tensor make it a tensor.
    ...
    expected_state_action_values=expected_state_action_values.unsqueeze(1).detach()
    ...
    with laser.Laser() as l:
        ...
        for i_episode in range(num_episodes):
                ...
                if(state_cpu.shape[0] != 0):
                    ...
                    # time.sleep(0.01)
                        ...
                        if terminated:
                            next_state = None
                            # if(distance >= 10):
                            #     reward = torch.tensor([-20], device=device)
                            # elif(distance <10):
                            #     reward = torch.tensor([20], device=device)
                        else:
                            next_state = observation.to(device)

                        # Store the transition in memory
                        # if(tensor_ag.shape !=(1,7056)):
                        #     print(tensor_ag.shape)
                        # if(next_state != None and next_state.shape != (1,7056)):
                        #     print(next_state.shape)
                        # if(done):
                        #     memory.push(tensor_ag.to(device), action, next_state, reward)
                        # else:
                        memory.push(tensor_ag.to(device), action, next_state, reward)
                        ...
                        if(loss is not None):
                            avg_loss += loss.item()
                        # if ((i_episode) % TARGET_UPDATE ==0) and (it==0):
                        #     target_net.load_state_dict(policy_net.state_dict())
            ...
            if ((i_episode) % TARGET_UPDATE ==0):
                target_net.load_state_dict(policy_net.state_dict())
                print("PARAMETERS JUST GOT UPDATED!")
            model_save_name = 'mod10.ext'
            path = "/opt/rl/antony/" + model_save_name
